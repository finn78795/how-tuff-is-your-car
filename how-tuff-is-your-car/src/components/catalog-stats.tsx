...
export function CatalogStats() {
  ...
  return (
    <div className="mt-4 flex flex-wrap justify-center gap-x-5 gap-y-2 text-xs font-bold text-zinc-600">
      <span className="flex items-center gap-1.5"><Database className="size-3.5" /> {stats ? `${stats.totalSearchable.toLocaleString()}+ searchable entries` : "Large multi-source catalog"}</span>
      <span className="flex items-center gap-1.5"><Landmark className="size-3.5" /> classics from 1950</span>
      <span className="flex items-center gap-1.5"><Sparkles className="size-3.5" /> opinion-based scores</span>
    </div>
  );
}
