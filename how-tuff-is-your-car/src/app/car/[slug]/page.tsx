      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        ...
        <section className="mt-7 grid gap-6 lg:grid-cols-[1.08fr_.92fr] lg:items-stretch">
          ...
          <div className="flex flex-col rounded-[2rem] border border-white/8 bg-[#101216] p-6 sm:p-8">
            ...
            <div className="mt-9 flex flex-1 flex-col justify-center">
              <p className="text-sm font-black uppercase tracking-[0.24em] text-lime-300">My take</p>
            ...
            <div className="mt-8 flex items-end justify-between gap-4 border-t border-white/8 pt-6">
              <div><p className="text-3xl font-black tracking-[-0.05em] text-lime-300">{getVerdict(overall)}</p><p className="mt-1 text-sm text-zinc-600">Overall, just for fun</p></div>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.12fr_.88fr]">
          <div className="rounded-[1.8rem] border border-white/8 bg-[#101216] p-6 sm:p-8">
            <div className="flex flex-wrap items-center gap-2"><p className="eyebrow">Why it landed here</p><span className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[.14em] text-zinc-500">{car.ratingConfidence} confidence</span></div>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl border border-lime-300/15 bg-lime-300/[0.045] p-4"><div className="flex items-center gap-2 text-sm font-black text-lime-300"><Sparkles className="size-4" /> What makes it cool</div><p className="mt-2 text-2xl font-black text-white">{ratingLabels[topRating]} · {car.ratings[topRating].toFixed(1)}</p></div>
              <div className="rounded-2xl border border-white/8 bg-white/[0.025] p-4"><div className="flex items-center gap-2 text-sm font-black text-zinc-500"><Gauge className="size-4" /> What holds it back</div><p className="mt-2 text-2xl font-black text-white">{ratingLabels[lowRating]} · {car.ratings[lowRating].toFixed(1)}</p></div>
            </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-[1.8rem] border border-white/8 bg-[#101216] p-6 sm:p-8">
              <p className="eyebrow">The quick stuff</p>
            ...
            <div className="rounded-[1.8rem] border border-white/8 bg-[#101216] p-6">
              <div className="flex items-center gap-2 text-sm font-black text-white"><Database className="size-4 text-lime-300" /> Where the details came from</div>
              ...
              {car.sourceUrl && <a href={car.sourceUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 text-sm font-black text-lime-300 hover:text-lime-200">See the source <ExternalLink className="size-3.5" /></a>}
              <div className="mt-4 flex gap-2 rounded-xl border border-white/7 bg-white/[0.02] p-3 text-xs leading-5 text-zinc-600"><BadgeInfo className="mt-0.5 size-4 shrink-0" /> This is an opinion score, not a value estimate, safety rating, or reason to buy one.</div>
            </div>
          </aside>
        </section>

        {related.length > 0 && <section className="py-16"><div className="mb-6 flex items-end justify-between"><div><p className="eyebrow">Same rabbit hole</p><h2 className="section-title">Cars with a similar vibe.</h2></div><Link href="/#browse" className="hidden items-center gap-2 text-sm font-black text-lime-300 sm:flex">Browse all <ArrowRight className="size-4" /></Link></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{related.map((item) => <CarCard key={item.slug} car={item} />)}</div></section>}
      </div>
    </main>
  );
}
